...
// netlify/functions/save-blog.ts
var save_blog_exports = {};
__export(save_blog_exports, {
  handler: () => handler2
});
module.exports = __toCommonJS(save_blog_exports);
...
// netlify/functions/save-blog.ts
var handler2 = async (event, context) => {
  ...
  try {
    ...
    let parsedYaml;
    try {
      parsedYaml = jsYaml.load(decodedContent);
    ...
    const isDuplicate = parsedYaml.blog.some(
      (post) => post.title.toLowerCase() === blogData.title.trim().toLowerCase() && post.date === (/* @__PURE__ */ new Date()).toISOString().split("T")[0]
    );
    if (isDuplicate) {
      return {
        statusCode: 409,
    ...
    const rawSlug = blogData.title.trim().toLowerCase().replace(/[^\w\s-]/g, "").replace(/[\s_-]+/g, "-").replace(/^-+|-+$/g, "");
    const newPost = {
      id: highestId + 1,
    ...
    try {
      updatedYamlContent = jsYaml.dump(parsedYaml, { indent: 2, lineWidth: -1 });
    } catch (e) {
...
*/
//# sourceMappingURL=save-blog.js.map
