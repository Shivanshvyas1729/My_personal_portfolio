...
const parseToHsl = (colorStr: string): { h: number; s: number; l: number } => {
  ...
  if (str.startsWith('#') || /^[0-9a-f]{3,6}$/.test(str)) {
    return hexToHsl(str);
  }
  ...
  return { h: 200, s: 95, l: 70 }; // default fallback
};

const adjustColor = (colorStr: string, mode: 'mobile' | 'tablet' | 'desktop', isDark: boolean): string => {
...
const EdgeRopeLight = () => {
  ...
  const config = useMemo(() => {
    ...
    // Scale opacity way down in light mode to make it soft and clean
    let glowOpacityMinMult = isDark ? 0.04 : 0.012;
    let glowOpacityMaxMult = isDark ? 0.08 : 0.028;

    if (mode === 'mobile') {
      glowMultiplier = isDark ? 1.5 : 1.1;
      speedMultiplier = 1.3; // 30% slower
      thicknessMultiplier = 1.35;
      glowOpacityMinMult = isDark ? 0.06 : 0.022;
      glowOpacityMaxMult = isDark ? 0.12 : 0.045;
    } else if (mode === 'tablet') {
      glowMultiplier = isDark ? 1.2 : 1.05;
      speedMultiplier = 1.15; // 15% slower
      thicknessMultiplier = 1.15;
      glowOpacityMinMult = isDark ? 0.05 : 0.016;
      glowOpacityMaxMult = isDark ? 0.09 : 0.035;
    }
    ...
    const adjustedColors = colors.map(c => adjustColor(c, mode, isDark));
    const adjustedSharpColors = sharpColors.map(c => adjustColor(c, mode, isDark));
